...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(['App','Category']).mean().sort_values(by='Reviews',ascending=False).head(10).reset_index()[['Category','App','Reviews','Rating']].to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])}
    ...
    ##improv
    X=pd.DataFrame(df2.groupby('Category').mean()['Price'].sort_values(ascending=False).head(3))
    X.reset_index(inplace=True)
    fig = plt.figure(figsize=(10,10))
    bar=plt.bar(X['Category'],X['Price'])
    plt.title("Top 3 Category With The Highest Mean Price",fontsize=20)
